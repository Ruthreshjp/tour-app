...
const GoogleMapsPickupPicker = ({ isOpen, onClose, onConfirm }) => {
  ...
  const createMap = (center) => {
    ...
    try {
      ...
      // This allows page scrolling when not hovering over the map
      if (mapRef.current) {
  ...
  const placeMarker = async (latlng) => {
    ...
    // Get address from coordinates using multiple services for better accuracy
    try {
      ...
      if (data && data.address) {
        ...
        toast.success('📍 Location pinned! Review the address below.');
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-4 overflow-y-auto">
      <div className="bg-white rounded-lg max-w-4xl w-full my-4 flex flex-col">
        ...
        <div className="p-4 border-b border-gray-200 flex justify-between items-center">
          <h3 className="text-xl font-semibold text-gray-800">📍 Pin Your Pickup Location</h3>
          <button
            onClick={handleClose}
        ...
        <div className="p-4 bg-blue-50 border-b border-blue-200">
          <p className="text-sm text-blue-800 font-medium mb-2">
            ...
            How to select your pickup location:
          </p>
          <ol className="text-sm text-blue-700 ml-6 list-decimal space-y-1">
            <li><strong>Click "Use Current Location"</strong> button on the map to pin your current location, OR</li>
            <li><strong>Click anywhere on the map</strong> to manually drop a pin at your exact pickup location</li>
            <li>The address will be automatically detected and displayed below</li>
            <li>Review the address and click "Confirm Location" when ready</li>
          </ol>
        </div>
        ...
        {tempLocation && (
          <div className="p-4 bg-green-50 border-t border-green-200">
            <p className="text-sm font-medium text-green-900 mb-2 flex items-center gap-2">
              ...
              <span>Selected Pickup Location:</span>
            </p>
            <div className="bg-white rounded-lg p-3 border border-green-200">
              <p className="text-sm text-gray-800 font-medium mb-1">{tempLocation.address}</p>
              {tempLocation.fullAddress && tempLocation.fullAddress !== tempLocation.address && (
                <p className="text-xs text-gray-600 mt-2 pt-2 border-t border-gray-200">
                  Full address: {tempLocation.fullAddress}
                </p>
              )}
              <p className="text-xs text-gray-500 mt-2 font-mono">
                📌 GPS: {tempLocation.lat.toFixed(6)}, {tempLocation.lng.toFixed(6)}
              </p>
            </div>
            <p className="text-xs text-green-700 mt-2 italic">
              ✓ Review the address above. You can click elsewhere on the map to change the location.
            </p>
          </div>
        )}
...
export default GoogleMapsPickupPicker;
